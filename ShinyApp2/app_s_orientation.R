  ...
  so_data <- reactive({
    la_so %>%
      filter(
        ...
        if ("All" %in% input$urb_rur_so) TRUE else urb_rur %in% input$urb_rur_so,
        if ("All" %in% input$reg_nam_so) TRUE else reg_nam %in% input$reg_nam_so
      )
  })
